...
import * as strings from "ReactScriptEditorWebPartStrings";
import ReactScriptEditor from "./components/ReactScriptEditor";
import { IReactScriptEditorProps } from "./components/IReactScriptEditorProps";
...
export default class ReactScriptEditorWebPart extends BaseClientSideWebPart<IReactScriptEditorWebPartProps> {
  ...
  protected async loadPropertyPaneResources(): Promise<void> {
    ...
    this._propertyPaneHelper = editorProp.PropertyFieldCodeEditor(
      "scriptCode",
      {
  ...
  private evalScript(elem) {
    ...
    scriptTag.type =
      (scriptTag.src && scriptTag.src.length) > 0 ? "pnp" : "text/javascript";
  ...
  private async executeScript(element: HTMLElement) {
    ...
    for (let i = 0; i < scriptTags.length; i++) {
      ...
      if (
        scriptTag.hasAttribute("pnpname") &&
        scriptTag.attributes["pnpname"].value == this._unqiueId
      ) {
